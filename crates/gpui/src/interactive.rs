...
pub trait MouseEvent: InputEvent + Clone {
    /// Returns this event with its position mapped to source coordinates.
    fn map_position(&self, map: impl Fn(Point<Pixels>) -> Point<Pixels>) -> Self;
}

macro_rules! impl_mouse_event {
    ($event:ty) => {
...
impl_mouse_event!(MouseDownEvent);
...
impl_mouse_event!(MousePressureEvent);
...
impl_mouse_event!(MouseMoveEvent);
...
impl_mouse_event!(ScrollWheelEvent);
...
impl_mouse_event!(MouseExitEvent);
...
impl MouseEvent for FileDropEvent {
    fn map_position(&self, map: impl Fn(Point<Pixels>) -> Point<Pixels>) -> Self {
        match self {
            Self::Entered { position, paths } => Self::Entered {
                position: map(*position),
                paths: paths.clone(),
            },
            Self::Pending { position } => Self::Pending {
                position: map(*position),
            },
            Self::Submit { position } => Self::Submit {
                position: map(*position),
            },
