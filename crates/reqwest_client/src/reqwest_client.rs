...
impl ReqwestClient {
    ...
    fn client_for_redirect_policy(
        ...
        let mut clients = self
            .redirect_clients
            .lock()
            .map_err(|_| anyhow!("redirect client cache lock was poisoned"))?;
        if let Some(client) = clients.get(redirect_policy) {
        ...
        let mut builder = Self::builder().redirect(policy);
        ...
        clients.insert(redirect_policy.clone(), client.clone());
        Ok(client)
    }
    ...
    pub fn proxy_and_user_agent(proxy: Option<Url>, user_agent: &str) -> anyhow::Result<Self> {
        ...
        client.use_preconfigured_tls = true;
        Ok(client)
    }
}
...
impl From<reqwest::Client> for ReqwestClient {
    fn from(client: reqwest::Client) -> Self {
        ...
        Self {
            client,
            redirect_clients: Mutex::new(HashMap::new()),
            handle,
            proxy: None,
            user_agent: None,
            use_preconfigured_tls: false,
        }
    }
}
...
impl http_client::HttpClient for ReqwestClient {
    ...
    > {
        ...
        let client = match parts.extensions.get::<RedirectPolicy>() {
            Some(redirect_policy) => self.client_for_redirect_policy(redirect_policy),
